...
#===================================#
  ...
  setnames(a_studs_added_avg, c("EOS Start Year", "Data Year", "Total Schools", "Avg. Students Added to AP/IB Over Baseline", 
                                 "Avg. Underrepresented Students Added", "Avg. Benchmark Students Added", "Avg. Percent Underrepresented"))
...
```{r}
  ...
  selected_data_p3a <- reactive({ output <- subset(a_studs_added_avg, chmatch(`EOS Start Year`, c(input$selected_cohort_p2), nomatch = 0) != 0 & 
                                                       chmatch(`Data Year`, c(input$selected_yr_p2), nomatch = 0) != 0) })

  # combine the selected variables into a new data frame - box and whisker
  selected_data_p3b <- reactive({ output <- subset(studs_added_wide, num_ap_students_added > 20 & 
  ...
  selected_data_p3c <- reactive({ output <- subset(studs_added_wide, num_ap_students_added > 20 & 
                                                       chmatch(start_year_with_eos, c(input$selected_cohort_p2), nomatch = 0) != 0 & 
...
```{r}
  ...
  # render shiny table
  output$table_st_add_avg <- renderDataTable(selected_data_p3a())
...
```{r}
  ...
  selected_data_p4 <- reactive({ subset(studs_added_wide, num_ap_students_added > 20 & chmatch(data_yr, c(input$selected_yr_p3), nomatch = 0) != 0) })
